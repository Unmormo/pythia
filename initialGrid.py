...
'''
########################################################################################################################无用代码
def patchupdata(m,template_h,template_x,template_y,h0,x0,y0,cdatabase,database,N):
    o1=template1(m,template_h,template_x,template_y,h0,x0,y0)#待模拟区
    #print o1
    #判断是哪个重叠区
    if o1[0,template_x/2,template_y/2]==0:#使用无上重叠区模板
        if (o1[template_h-1,0,template_y/2]!=0) and (o1[template_h-1,template_x/2,0]!=0) and (o1[template_h-1,template_x-1,template_y/2]==0) and (o1[template_h-1,template_x/2,template_y-1]==0):#左后
            tem=patternsearch1(cdatabase[0],database,o1,N)#左后
            template1R(m,tem,h0,x0,y0)
        if (o1[template_h-1,template_x-1,template_y/2]!=0) and (o1[template_h-1,template_x/2,0]!=0) and (o1[template_h-1,template_x/2,template_y-1]==0) and (o1[template_h-1,0,template_y/2]==0):#左前
            tem=patternsearch1(cdatabase[1],database,o1,N)#左前
            template1R(m,tem,h0,x0,y0)
        if (o1[template_h-1,template_x-1,template_y/2]!=0) and (o1[template_h-1,template_x/2,template_y-1]!=0) and (o1[template_h-1,0,template_y/2]==0) and (o1[template_h-1,template_x/2,0]==0):#右前
            tem=patternsearch1(cdatabase[2],database,o1,N)#右前
            template1R(m,tem,h0,x0,y0)
        if (o1[template_h-1,0,template_y/2]!=0) and (o1[template_h-1,template_x/2,template_y-1]!=0) and (o1[template_h-1,template_x-1,template_y/2]==0) and (o1[template_h-1,template_x/2,0]==0):#右后
            tem=patternsearch1(cdatabase[3],database,o1,N)#右后
            template1R(m,tem,h0,x0,y0)
        if (o1[template_h-1,0,template_y/2]!=0) and (o1[template_h-1,template_x/2,template_y-1]!=0) and (o1[template_h-1,template_x-1,template_y/2]!=0):
            tem=patternsearch1(cdatabase[4],database,o1,N)
            template1R(m,tem,h0,x0,y0)#右前后
        if (o1[template_h-1,0,template_y/2]!=0) and (o1[template_h-1,template_x/2,0]!=0) and (o1[template_h-1,template_x-1,template_y/2]!=0):
            tem=patternsearch1(cdatabase[5],database,o1,N)
            template1R(m,tem,h0,x0,y0)#左前后
        if (o1[template_h-1,0,template_y/2]!=0) and (o1[template_h-1,template_x/2,0]!=0) and (o1[template_h-1,template_x/2,template_y-1]!=0):
            tem=patternsearch1(cdatabase[6],database,o1,N)
            template1R(m,tem,h0,x0,y0)#左右后
        if (o1[template_h-1,template_x-1,template_y/2]!=0) and (o1[template_h-1,template_x/2,0]!=0) and (o1[template_h-1,template_x/2,template_y-1]!=0):
            tem=patternsearch1(cdatabase[7],database,o1,N)
            template1R(m,tem,h0,x0,y0)#左右前
    else:#有上重叠区时
        if (o1[template_h-1,0,template_y/2]!=0) and (o1[template_h-1,template_x/2,0]!=0) and (o1[template_h-1,template_x-1,template_y/2]==0) and (o1[template_h-1,template_x/2,template_y-1]==0):#左后
            tem=patternsearch1(cdatabase[8],database,o1,N)#左后
            template1R(m,tem,h0,x0,y0)
...
def patchupdataB(m,template_h,template_x,template_y,d1,cdatabase,database,N):#并行version
    for n in range(len(d1)):
        o1=template1(m,template_h,template_x,template_y,d1[n][0],d1[n][1],d1[n][2])#待模拟区
        #print o1
        #判断是哪个重叠区
...
def initializeGrid(m,template_h,template_x,template_y,lag_h,lag_x,lag_y,database,zuobiaolist,N):#非柱状初始模型构建,N为备选模式个数
    d1=[]
    lujing1(m,d1,template_h,template_x,template_y,lag_h,lag_x,lag_y)
    #print d1
    cdatabase=temdatabasec_5(database,1,template_h,template_x,template_y)#重叠区宽度为1
    for n in range(len(d1)):
        o1=template1(m,template_h,template_x,template_y,d1[n][0],d1[n][1],d1[n][2])#待模拟区
        #print o1
        #判断是哪个重叠区
...
def initializeGrid2(m,template_h,template_x,template_y,lag_h,lag_x,lag_y,database,zuobiaolist,N):#非柱状初始模型构建,N为备选模式个数
    d1=[]
    lujing1(m,d1,template_h,template_x,template_y,lag_h,lag_x,lag_y)
    #print d1
    cdatabase=temdatabasec_5(database,1,template_h,template_x,template_y)#重叠区宽度为1
    for n in range(len(d1)):
        ...
        x0=d[n][1]
        y0=d[n][2]
        patchupdata(m,template_h,template_x,template_y,h0,x0,y0,cdatabase,database,N)
    return m
'''
