...
def subrandom_searchEM(A,f, a, dist, A_padding,  Cdatabase,zuobiaolist,p_size,q):#随机搜索
    ...
    arrylist2=[]
    for n in range(len(Cdatabase[idx])):
        Ti=template1(A_padding,p_size,p_size,p_size,zuobiaolist[Cdatabase[idx][n]][0]+p,zuobiaolist[Cdatabase[idx][n]][1]+p,zuobiaolist[Cdatabase[idx][n]][2]+p)
        distcal= calDIST(a,A_padding, Ti, p_size)
        arrylist2.append(distcal)
    idy = np.argmin (arrylist2)
    a=np.zeros((2),int)
    a[0]= arrylist2[idy]
...
Tidatabase=np.load('./database/Tidatabase.npy')
zuobiaolist=np.load('./database/zuobiaolist.npy')
print len(Tidatabase),len(zuobiaolist),125*300*2+150*125*2
'''
'''
def wtfa(n,q):
    a=np.zeros((2),int)
...
q = multiprocessing.Queue()
for n in range(20):
    s=multiprocessing.Process(target=wtfa, args=(n,q)) 
    print('process:',n)
    s.start()
    processes.append(s)
for s in processes:
...
c[0,:,:]=3
print(c)
lista=patternSelhi(a)
listb=patternSelhi(b)
listc=patternSelhi(c)
print(lista )
print (listc)
listdatabase=[]
listcompair(lista,listdatabase)
listcompair(listb,listdatabase)
#listcompair(listc,listdatabase)
print(listdatabase)
if listdetect(listc,listdatabase):
    print('done')
print( listdetect2(lista,listdatabase))
list1=[[],[],[],]
list1[1].append(1)
print(list1)
'''
